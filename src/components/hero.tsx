...
interface HeroProps {
  onBookClick: (presetService?: string) => void;
}
...
export default function Hero({ onBookClick }: HeroProps) {
  ...
  return (
    <section id="hero" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-28 pb-10">
      {/* Curved Container Matching Mockup */}
      <div className="bg-[#f2f4f7] rounded-[2.5rem] p-8 md:p-12 lg:p-16 relative overflow-hidden border border-border/40">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-center">
          
          {/* Left Column */}
          <div className="lg:col-span-7 flex flex-col items-start text-left space-y-6 md:space-y-8">
            
            {/* Top Avatar + Count Row */}
            ...
            <button
              onClick={() => onBookClick()}
              className="px-8 py-4 rounded-full bg-accent text-accent-foreground font-black text-sm hover:bg-accent/90 transition-all duration-200 cursor-pointer shadow-sm hover:shadow"
            >
              {t.bookBtn}
            </button>

            {/* Bottom Experience and Rating Grid */}
          ...
          {/* Right Column - Bangladeshi / South Asian Doctor portrait */}
